...
function QuickInspector({ enabled, toggleEnabled }: QuickTeamSearchProps) {
    ...
    return (
        <>
            ...
            <Modal transition={'rotate-left'} centered zIndex={10001} opened={enabled} onClose={() => {
                toggleEnabled()
                close()
                ...
                <div ref={focusTrapRef}>
                    <TextInput
                        ...
                        onKeyDown={(event) => {
                            if (!searchbyteamnum) return
                            if (event.code !== 'Enter') return
                            if (visible) {
                                setSearchbyteamnum('')
                                return showNotification({
                                    title: 'Error!',
                                    message: 'Teams are still loading!',
                                    color: theme.colors.red[6],
                                })
                            }
                            if (teams.length !== 0 && teams.filter((team: any) => team.number == searchbyteamnum).length == 0) {
                                setSearchbyteamnum('')
                                return showNotification({
                                    title: 'Error!',
                            ...
                            //@ts-ignore
                            setSelectedTeam({ content: searchbyteamnum })
                            setSearchbyteamnum('')
                            toggleEnabled()
                        }}
                    />
            ...
            <div className="DrawerContent" style={{ position: 'relative' }}>
                ...
                {pitScoutingData && pitScoutingData?.robotImage && (<Drawer
                    zIndex={10000}
                    opened={(opened && !enabled) ? true : false}
                    onClose={close}
